...
namespace Common.WindowStyle
{
    public partial class WindowCustomBorderStyle : ResourceDictionary
    {
        public WindowCustomBorderStyle()
        {
            ...
            if (vs.Major <= 5)
            {
                osDependantResources.Source = new Uri("/Resources/Themes/WindowStyleXP.xaml", UriKind.Relative);
            }
            else
            {
                osDependantResources.Source = new Uri("/Resources/Themes/WindowStyleWin7.xaml", UriKind.Relative);
            }
        ...
        private void UpdateWindowConstraints(Window window)
        {
            if (window != null)
            {
                ...
                if (borderWidth != null)
                {
                    window.MaxHeight = SystemParameters.WorkArea.Height * 0.7; //SystemParameters.WorkArea.Height + borderWidth.Value * 2;
                    window.MaxWidth = SystemParameters.WorkArea.Width * 0.57; //SystemParameters.WorkArea.Width + borderWidth.Value * 2;
                }
            }
        }
        ...
        private void OnThemeSelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            if (e.AddedItems.Count > 0)
            {
                ...
                skin.Source = new Uri(@"Resources\Themes\Skins\" + themeXamlFileName + ".xaml", UriKind.Relative);
